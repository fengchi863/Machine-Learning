...
"""
Created on Tue Aug 27 17:11:31 2019
    ...
    def train(self, X_train, y_train, learning_rate=0.005, n_epoch=5):
        ...
        for epoch in range(n_epoch):
            for idx in range(len(y_train)):
                self.sgd_step(X_train[idx], y_train[idx], learning_rate)
                num_examples += 1
            ...
            if len(losses) > 1 and losses[-1] > losses[-2]:
                learning_rate *= 0.5
                print('decrease learning_rate to' + str(learning_rate))
                    
# get data
file_path = r'../RNN/results-20170508-103637.csv'
dict_size = 8000
...
# train LSTM
lstm = LSTM(dict_size, hidden_dim=100)
